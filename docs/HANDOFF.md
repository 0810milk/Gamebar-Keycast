...
原唯一阻塞项「固定后只显示按键」已于 2026-08-16 定位根因并修复（见 `docs/ISSUE-PINNED-CHROME.md` 第 9 节）：
根因是激活时 `GameBarDisplayMode` 误报 `PinnedOnly` 而代码只按 mode 判定；修复为
`mode==PinnedOnly && Pinned` 复合判定 + 每个实例私有 widget 引用 + 固定高度按钮行。
