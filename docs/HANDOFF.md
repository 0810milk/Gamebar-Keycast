...
第 9–13 节）；2026-08-16 另放开帧率（第 14 节）、鼠标垫跟随屏幕纵横比（第 15 节）、
空格键移第四行（第 16 节）、全屏触边改用**活动范围自适应**（第 17 节，
协议升级 v3=44B，新增归一化 ux/uy；伴生进程滑动窗口归一化，widget 按 ux/uy 渲染）。
**待用户实测**：固定后退出只留按键、主题切换不崩、游戏中鼠标点平滑跟随且能触垫边、
高刷流畅、垫子比例随屏幕变化。确认后按需调整 `fps`（config.json），重建 `Setup.exe` 并 commit；
