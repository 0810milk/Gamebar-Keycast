...
> **当前最新交接事项：0.5.0（规划中）控件吸附对齐功能**——拖拽/缩放时按周边控件十字方向边对边网格吸附 + 浅蓝虚线参考线。
> 任务书：`docs/TASK-0.5.0-snap.md`（自包含，含验收点），待派发执行。
> ⚠️ 工作区干净（已含 0.4.1 安装器修复 / MIT LICENSE / GitHub 发布流程 / Agent 继承协议等历史提交）。
