...
> 当前版本：**0.9.0 beta**，版本登记见 [VERSION.md](VERSION.md)。
...
| 版本 | 日期 | 更新内容 |
|---|---|---|
| **0.9.0 beta** | 2026-08-20 | 右键菜单完整交互（删除/复制/改名/粘贴）+ 按键改名与显示名体系（重置恢复/预设携带）+ 内置默认布局更新（Tab 56×48 + 鼠标垫 94,0/223.6×139.8 + 尺寸污染自动修复）+ 子菜单窗口自适应（滚动/缩宽/叠放）+ 整体动画（弹层淡入/启动淡入）+ 关于页链接（伴生进程 OPEN_URL 通道 + 剪贴板兜底）+ 主题调色盘修复（拖色相出纯色/不再切回红）+ 浮层派生配色 + 遮罩透明化 + 改名面板背板/重置恢复配色/光标修复等（UWP 包 1.20.0.0） |
| **0.8.0 beta** | 2026-08-19 | 内置默认布局预设（首次启动自动套用 + 「重置布局」恢复到此预设）+ 布局预设同步鼠标垫尺寸/位置（不同步比例，高度按本机屏幕重算）+ 自定义键尺寸精确还原 + 色相条「色相」文字/选色回退修复 + 移除冗余导入提示 + RESP 前缀解析修复（UWP 包 1.5.0.0） |
| **0.7.0 beta** | 2026-08-19 | 用户预设（主题/布局，包外 presets.json 卸载不丢）+ 自定义按键新增鼠标/滚轮键（滚轮 150ms 点亮）+ 鼠标垫右侧滚轮键 + 显示名统一 + 预设子菜单简化 + 配色联动/调色编号同步修复 + bar 内透明度修复 + 安装升级优先不丢数据（UWP 包 1.4.0.0） |
| **0.6.0 beta** | 2026-08-19 | 五态主题（黑/灰/白/粉/蓝循环、按钮显示当前色名）+ 主题颜色自定义子菜单（8 槽位调色盘：色相/透明度/常用色块/hex、custom 持久化）+ 关于面板 + 调色盘拖动流畅优化 + 注释还原（UWP 包 1.3.0.0） |
